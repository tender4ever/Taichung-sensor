...
mysql_select_db($database_cussensor, $cussensor);
$query_Recordset2 = sprintf("SELECT * FROM sensor WHERE customer_id = %s", GetSQLValueString($column_Recordset2, "int"));
...
<p>感測器 :  
    <select name="sensor_id" id="sensor_id" style="width: auto">
     <?php
		do {  
		?>
      	<option value="<?php echo $row_Recordset2['sensor_id']?>"<?php if (!(strcmp($row_Recordset2['sensor_id'], $row_Recordset2['sensor_id']))) {echo "selected=\"selected\"";} ?>>
						<?php echo $row_Recordset2['site']?></option>
     <?php
			} 
