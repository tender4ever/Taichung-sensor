...
$query_Recordset2 = sprintf("SELECT * FROM sensor WHERE customer_id = %s", GetSQLValueString($colname_Recordset2, "int"));
...
<form id="form1" name="form1" method="post" action="">
  ...
  <?php do { ?>
    <?php if ($totalRows_Recordset2 > 0) { // Show if recordset not empty ?>
      <p>位址:<?php echo $row_Recordset2['site']; ?>
        <input name="hiddenField2" type="hidden" id="hiddenField2" value="<?php echo $row_Recordset2['sensor_id']; ?>" />
      <a href="sensorset.php?sensor_id=<?php echo $row_Recordset2['sensor_id']; ?>">修改</a> <a href="delsensor.php?sensor_id=<?php echo $row_Recordset2['sensor_id']; ?>">刪除</a></p>
