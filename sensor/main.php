...
$query_Recordset1 = sprintf("SELECT * FROM customer WHERE account = %s", GetSQLValueString($colname_Recordset1, "text"));
...
mysql_select_db($database_sensor, $sensor);
$query_Recordset2 = sprintf("SELECT * FROM sensor WHERE customer_id = %s ORDER BY sensor_id ASC", GetSQLValueString($colname_Recordset2, "int"));
...
<form id="form1" name="form1" method="post" action="">
  ...
  <p> 單位 : <a href="nameset.php"><?php echo $row_Recordset1['name']; ?></a>
    <input name="customer_id" type="hidden" id="customer_id" value="
<?php echo $row_Recordset1['customer_id']; ?>" />
    <?php 
session_start();
$_SESSION['customer_id']=$row_Recordset1['customer_id'];
?>
  <a href="main.php"><img src="123.png" width="35" height="35" /></a></p>
  <hr />
  <?php do { ?>
  	<fieldset style="width:310px; height:230px; margin:auto">
    <legend> 
    <p style="font-size:20px">位置:<?php echo $row_Recordset2['site']; ?> </p>
    </legend>
    ...
    <a href="delsetting.php?sensor_id=<?php echo $row_Recordset2['sensor_id']; ?>">關閉警示</a>
      <iframe src="main1.php?sensor_id=<?php echo $row_Recordset2['sensor_id'];?>" width="300" height="180" scrolling="no" frameborder="0"></iframe>
